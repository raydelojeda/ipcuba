...
{		
	...
	for($dpa=1;$dpa<=$cant_dpa;$dpa++)
	{
		...
		and captacion.fecha<'$fecha_cal_inicio_sem1_next'";//print $sql_sel_variedad;die();
		...
		{ 
			...
			if($cant_cap>0)
			{
				...
				{
					$representatividad=($count+1)/$cant_cap;//print $representatividad."=";print $count."/";print $cant_cap;die();
			...
			and captacion.fecha<'$fecha_cal_inicio_sem1_next' and captacion.precio=0";//$k=$k+1;print $sql_cap."<br>".$k;die();
				...
				if($relativos)foreach ($relativos as $g => $valor) {unset($relativos[$g]);}
				if($precios_ant)foreach ($precios_ant as $g => $valor) {unset($precios_ant[$g]);}
