...
if($_POST['txt_ir'])
{//print "sds";
header("Location:indice_var_dpa.php?curr_page=".$_POST['txt_ir']."&order=".$order."&type=".$ordtype."&txt_filtro=".$txt_filtro."&sel_filtro=".$sel_filtro."&ver=".$ver."&sel_cod_dpa=".$sel_cod_dpa."&sel_mes=".$sel_mes."&sel_ano=".$sel_ano."&porc=".$porc);
}
if (isset($_GET["order"])) $order = $_GET["order"]; else $order="d_var_dpa.indice";
...
$sql_usuario = "select rol, id_usuario, usuario.cod_dpa,prov_mun,prov_mun_nuevo from usuario,n_dpa".$query_usuario;	
//print 	$sql_usuario;
...
$prov_mun_nuevo=$rs_usuario->Fields("prov_mun_nuevo");
...
$query = "select n_variedad.id_variedad, ecod_var, variedad, cod_dpa_nueva, prov_mun_nuevo,
d_var_dpa.indice, n_mercado.id_mercado, mercado
from n_variedad, b_variedad, d_var_dpa, n_dpa, n_mercado
where d_var_dpa.fecha='".$fecha_cal_inicio_sem1_next."' and d_var_dpa.idb_variedad=b_variedad.idb_variedad 
and n_dpa.cod_dpa=d_var_dpa.cod_dpa and b_variedad.id_variedad=n_variedad.id_variedad 
and b_variedad.id_mercado=n_mercado.id_mercado ";
                    ...
                    <td class="menudottedline" align="right"> <table width="100%" border="0" class="menubar"  id="toolbar">
                        <tr > 
                          ...
                          <td width="90%" valign="middle"  class="us"><strong><font color="#5A697E" size="3">Listado
                                 de variaciones de micro&iacute;ndices de precios por variedad y mercado
...
<?php }?>
                      ...
                      <table width="467" height="32" border="0" cellpadding="0" cellspacing="0" class="filtro" >
                        <tr>
                  ...
                  <tr align="center" valign="center"  > 
                    <td class="intro" width="25" height="20">No</td>
                    <td width="110" class="intro" ><a href="indice_var_dpa_x_mer.php?order=<?php echo  "mercado".", ".$order;?>&type=<?php echo $ordtypestr ?>&txt_filtro=<?php echo $txt_filtro?>&sel_filtro=<?php echo $sel_filtro?>&ver=<?php echo $ver?>&sel_cod_dpa=<?php echo $sel_cod_dpa?>&sel_ano=<?php echo $sel_ano?>&sel_mes=<?php echo $sel_mes?>">Mercado</a></td>
                    <td width="113" class="intro" ><a href="indice_var_dpa_x_mer.php?order=<?php echo  "ecod_var";?>&type=<?php echo $ordtypestr ?>&txt_filtro=<?php echo $txt_filtro?>&sel_filtro=<?php echo $sel_filtro?>&ver=<?php echo $ver?>&sel_cod_dpa=<?php echo $sel_cod_dpa?>&sel_ano=<?php echo $sel_ano?>&sel_mes=<?php echo $sel_mes?>">Código Variedad</a></td>
                    <td width="312" class="intro" ><a href="indice_var_dpa_x_mer.php?order=<?php echo  "variedad";?>&type=<?php echo $ordtypestr ?>&txt_filtro=<?php echo $txt_filtro?>&sel_filtro=<?php echo $sel_filtro?>&ver=<?php echo $ver?>&sel_cod_dpa=<?php echo $sel_cod_dpa?>&sel_ano=<?php echo $sel_ano?>&sel_mes=<?php echo $sel_mes?>">Variedad</a></td>
                    <td width="94" class="intro" ><div align="right"><a href="indice_var_dpa_x_mer.php?order=<?php echo "d_var_dpa.indice";?>&type=<?php echo $ordtypestr;?>&txt_filtro=<?php echo $txt_filtro;?>&sel_filtro=<?php echo $sel_filtro;?>&ver=<?php echo $ver;?>&sel_cod_dpa=<?php echo $sel_cod_dpa;?>&sel_ano=<?php echo $sel_ano;?>&sel_mes=<?php echo $sel_mes;?>">Variación mensual (%)</a></div></td>
                     <td width="87" class="intro" ><?php if($rol!="autor") {?><a href="indice_var_dpa_x_mer.php?order=<?php echo "cod_dpa_nueva".", ".$order;?>&type=<?php echo $ordtypestr ?>&txt_filtro=<?php echo $txt_filtro?>&sel_filtro=<?php echo $sel_filtro?>&ver=<?php echo $ver?>&sel_cod_dpa=<?php echo $sel_cod_dpa?>&sel_ano=<?php echo $sel_ano?>&sel_mes=<?php echo $sel_mes?>">Cod DPA</a><?php }else print "&nbsp;";?></td>
...
{	
  ...
  ?>
                  <tr  height="50" <?php $a=$pager_nav->index_rs++;if($a % 2) print "class=\"row1\""; else print "class=\"row0\"";?>> 
                    ...
                    <td class="raya"align="center"><?php echo $rs->fields["ecod_var"];?></td>
                    <td class="raya"align="center">
					  <?php echo $rs->fields["variedad"];?></td>
