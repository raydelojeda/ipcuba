...
require_once("../coneccion/conn.php");
 ...
 if(!empty($_POST)){ // con esto evito el intento de acceder directamente a esta pagina escribiendo su URL en el browser
	...
	if(count($var) != 0)
  {
	...
	$query_rs_delete= "UPDATE captacion SET  precio='0', id_obs='5' WHERE $campo='".$Id."'";
	  
	  $rs_delete = $db->Execute($query_rs_delete) or die($db->ErrorMsg()."Este mensaje no es un error como tal, lo que sucede es que existen datos que dependen de lo que se quiere modificar. Dé click en el botón <- Atrás del navegador.");//print $a;print $query_rs_delete;	
	  if($rs_delete)
		{
