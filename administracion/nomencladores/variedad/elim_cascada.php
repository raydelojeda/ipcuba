 ...
 if(!empty($_POST)){ // con esto evito el intento de acceder directamente a esta pagina escribiendo su URL en el browser
	...
	if(count($var) != 0)
  {
  	for ($i = 0; $i < count($var); next($var), $i++) 
	{
		...
		$sql_bvariedad="select idb_variedad from b_variedad where $campo='".$Id."'";//print $sql_var_estab."<br>";
		$rs_bvariedad= $db->Execute($sql_bvariedad)or $mensaje=$db->ErrorMsg();
		$cant_bvariedadb=$rs_bvariedad->RecordCount(); 
		
		for($b=0;$b<$cant_bvariedadb;$b++)
		{	
			$idb_variedad=$rs_bvariedad->Fields("idb_variedad");
			
			$sql_var_estab="select id_var_estab from n_var_estab where idb_variedad='".$idb_variedad."'";//print $sql_var_estab."<br>";
			...
			$sql_del_var_estab="delete from n_var_estab where idb_variedad='".$idb_variedad."'";//print $sql_del_var_estab."<br>";
			$db->Execute($sql_del_var_estab)or die($db->ErrorMsg());
			
			$sql_del_d_var_dpa="delete from d_var_dpa where idb_variedad='".$idb_variedad."'";//print $sql_del_d_var_dpa."<br>";
			$db->Execute($sql_del_d_var_dpa)or die($db->ErrorMsg());
			
			$sql_del_d_var_dpa="delete from i_var_dpa where idb_variedad='".$idb_variedad."'";//print $sql_del_d_var_dpa."<br>";
			$db->Execute($sql_del_d_var_dpa)or die($db->ErrorMsg());
			
			$sql_del_b_variedad="delete from b_variedad where idb_variedad='".$idb_variedad."'";//print $sql_del_d_var_dpa."<br>";
			$db->Execute($sql_del_b_variedad)or die($db->ErrorMsg());
			
		$rs_bvariedad->MoveNext();
		}
