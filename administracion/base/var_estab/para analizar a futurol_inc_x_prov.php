...
$fecha_cierre_sem4_1=substr($fecha_base,0,8).$dia_base_ini;
...
$fecha_cal_inicio_sem_pasada=$rs_cal->fields["fecha_cal"];
//--------------------------------PARA OBETENER LA FECHA DEL INICIO DE SEMANA 1 DEL MES PASADO--------------------------------
...
$fecha_cierre_sem4_3=substr($fecha_base,0,8).$dia_base_ini;
...
$rs_cal = $db->Execute($sql_cal) or die($db->ErrorMsg());//print $sql_cal;//die();
$fecha_cal_inicio_sem_actual=$rs_cal->fields["fecha_cal"];
//--------------------------------PARA OBETENER LA FECHA DEL INICIO DE SEMANA 1 DEL MES ACTUAL--------------------------------
...
//print $dia_cal." - ".$dia_captar." - ";

if(($dia_captar-$dia_cal)>0)
{$dif=$dia_captar-$dia_cal;  $dia_cal_cierre=$miercoles_dia_cierre-$dif;}
else
{$dif=$dia_cal-$dia_captar;   
//print $dif." - ".$dia_cal." - ".$dia_captar;
$dia_cal_cierre=$miercoles_dia_cierre+$dif;}
//print  $dia_cal_cierre."   ".$miercoles_dia_cierre."   ".$dif."<br>";
$fecha_cal_cierre_sem=substr($rs_cal->fields["fecha_cal"],0,8).$dia_cal_cierre;

if($sem=="04" || $sem=="0")
$fecha_cal_cierre_sem=$fecha_cal_cierre_sem4;
///-----------------------------------------------------
...
and incluido='1' and central='0' and n_variedad.ide_articulo!='1' and fecha_creacion<'".$fecha_cal_cierre_sem."' and (fecha_desuso>'".$fecha_cal_cierre_sem."' or n_var_estab.desuso='0')
";
            ...
            <td align="center" valign="middle" bgcolor="#FFFFFF"> <div align="center"> 
                  ...
                  <tr> 
                    <td class="menudottedline" align="right"> <table width="100%" border="0" class="menubar" id="toolbar"  >
                        <tr  > 
                          ...
                          <td width="7%"> <div align="center"> <a  class="toolbar" href="exp_inc_x_prov.php?sem=<?php print $sem;?>&mes=<?php print $mes1;?>&ano=<?php print $ano1;?>"   target="_blank"> 
                ...
                <table width="99%" height="100" border="0" cellpadding="0" cellspacing="0" class="tabla" id="toolbar1">
                  ...
                  <tr align="center" valign="center"  > 
                    ...
                    <td width="10%" align="center" class="intro1"><a onMouseOver="return overlib('Cantidad de precios distribuidos a captar en el mes.', ABOVE, RIGHT);" onMouseOut="return nd();" class="toolbar1">Precios que se distribuyeron en el mes</a></td>
                    ...
                    <td width="10%" align="center" class="intro1"><a onMouseOver="return overlib('Porcentaje de precios observados respecto a los precios distribuidos en el mes.', ABOVE, RIGHT);" onMouseOut="return nd();" class="toolbar1">Porcentaje de observados respecto al total en fecha(%)</a></td>
  ...
  if($rs->fields[0]!='')
{
  ...
  if($mes1!=0 and $ano1!=0)
				  ...
				  and captacion.fecha>='".$fecha_cal_inicio_sem_actual."' and captacion.fecha<'".$fecha_cal_cierre_sem."' 
				  and fecha_creacion<'".$fecha_cal_cierre_sem."' and (fecha_desuso>'".$fecha_cal_cierre_sem."' or n_var_estab.desuso='0')
				  ...
				  and n_var_estab.id_var_estab=captacion.id_var_estab 
				  and n_var_estab.fecha_captar>='".$fecha_base_ini."' and n_var_estab.fecha_captar<='".$fecha_base_fin."' 
				  and captacion.fecha>='".$fecha_cal_inicio_sem_actual."' and captacion.fecha<'".$fecha_cal_inicio_sem1_next."' 
				  and fecha_creacion<'".$fecha_cal_inicio_sem1_next."' and (fecha_desuso>'".$fecha_cal_inicio_sem1_next."' or n_var_estab.desuso='0')			    
				  and n_dpa.cod_dpa='".$rs->fields["cod_dpa"]."' 
				  and captacion.precio='0'
			 	  group by n_dpa.cod_dpa order by n_dpa.cod_dpa";
				  //print $sql22."<br>";
				  ...
				  and captacion.precio='0' and id_inc='4'
			 	  group by n_dpa.cod_dpa order by n_dpa.cod_dpa";
				 // print $sql4;
				  $rs4 = $db->Execute($sql4)or die($db->ErrorMsg());//no visitados
				  ...
				  and fecha_creacion<'".$fecha_cal_cierre_sem."' and (fecha_desuso>'".$fecha_cal_inicio_sem1_next."' or n_var_estab.desuso='0')
			 	  group by n_dpa.cod_dpa order by n_dpa.cod_dpa";
				  //print $sql5;
				  $rs5 = $db->Execute($sql5)or die($db->ErrorMsg());//faltas
				  print $fecha_cal_cierre_sem;
				  if($_POST["sel_sem"]!="04")
				  {
				  ...
				  and n_var_estab.id_var_estab=captacion.id_var_estab 
				  and n_var_estab.fecha_captar>='".$fecha_base_ini."' and n_var_estab.fecha_captar<='".$fecha_base_fin."' 
				  and captacion.fecha>='".$fecha_cal_cierre_sem."' and captacion.fecha<'".$fecha_cal_inicio_sem1_next."' 
				  and fecha_creacion<'".$fecha_cal_inicio_sem1_next."' and (fecha_desuso>'".$fecha_cal_inicio_sem1_next."' or n_var_estab.desuso='0')
				  and n_dpa.cod_dpa='".$rs->fields["cod_dpa"]."'
			 	  group by n_dpa.cod_dpa order by n_dpa.cod_dpa";
				  print $sql6."<br><br>";
...
{$count22=$count2;$count6="";}
 ...
 $porc=$count22/($rs1->fields["count"]+$count6);//porcentaje no observados en fecha y fuera de fecha
...
if($cuba!="0" && $cuba!="")$porc6_cuba=round(($cuba1+$cuba6)/$cuba*100,1);//cumplimiento en fecha y fuera de fecha
  ?>
  				  ...
  				  <tr <?php $a=$pager_nav->index_rs++;if($a % 2) print "class=\"row1\""; else print "class=\"row0\"";?> >
  				    ...
  				    <td width="19%" align="center" class="raya">&nbsp;</td>
  				  ...
  				  <tr <?php $a=$pager_nav->index_rs++;if($a % 2) print "class=\"row1\""; else print "class=\"row0\"";?> >
                    <td height="21" colspan="2"align="center" class="raya"><div align="left"><strong>2300. C. Habana</strong></div></td>
